...
> **WARNING -- DESTRUCTIVE TOOL.** This script wipes storage contents, removes
> LVM volumes and ZFS datasets, purges packages, and can reset all PVE
> configuration to factory defaults. Data loss is intentional and
> irreversible. Always start with `--plan` or `--dry-run` before executing
> for real. Back up anything you need first.
...
- Plan and scope changes before execution.
...
- `--plan` mode: deterministic, no side effects.
...
Always review the plan before executing. The progression below goes from
read-only inspection to destructive execution.

1. **Review the plan** (read-only, no root required):

```bash
./pve-soft-reset.sh --plan
```

2. **Simulate execution** (read-only, shows what each phase would do):

```bash
./pve-soft-reset.sh --dry-run
```

3. **Execute for real** (DESTRUCTIVE -- wipes storage, removes volumes):
...
- JSON output is backward compatible; new fields (`meta`, `warnings`) are additive.
