...
Usage: ${c_opt}$0${c_rst} [OPTIONS]
  ...
  Without a mode flag, the script runs in ${c_warn}execute mode (DESTRUCTIVE)${c_rst}.
  Always start with --plan or --dry-run to review what would happen.

Modes (mutually exclusive, read-only unless noted):
  ${c_opt}--plan${c_rst}                       Show deterministic plan, no execution, no prompts.
  ...
  ${c_opt}--json${c_rst}                       Output audit result as JSON and exit.
...
Execution control:
  ${c_opt}-y, --yes${c_rst}                    Skip confirmation prompts (${c_warn}DANGEROUS${c_rst}).
  ...
  ${c_opt}--backup-config${c_rst}              Back up /etc/pve before reset operations.

Reset flags (${c_warn}DESTRUCTIVE${c_rst} -- each deletes or overwrites data):
  ${c_opt}--reset-pve-config${c_rst}           Delete guest configs, SDN, mappings, jobs, firewall, HA.
...
Third-party purge (${c_warn}DESTRUCTIVE${c_rst}):
