...
execute_reset_ssh_keys() {
  ...
  if $DRY_RUN; then
    ...
    sed -i -E '/root@|pve/d' "$auth_keys" || ((FAILURE_COUNT+=1))
  fi
}
...
run_execution_phases() {
  ...
  log_info "$(_phase_label "Starting storage wipe phase (dir)")"
  execute_wipe_dirs
  # Count dir storage entries processed (not individual subdirs wiped).
  DIRS_WIPED=$((DIRS_WIPED + ${#WIPE_DIR_ENTRIES[@]}))
  maybe_sync

  log_info "$(_phase_label "Starting storage wipe phase (LVM)")"
  execute_wipe_lvm
  add_sep_minus_one_to_var WIPE_LVM_ENTRIES LVS_REMOVED
  maybe_sync

  log_info "$(_phase_label "Starting storage wipe phase (ZFS)")"
  execute_wipe_zfs
  ...
  log_info "$(_phase_label "Running third-party cleanup")"
  execute_purge_third_party

  if $RESET_PVE_CONFIG; then
    log_info "$(_phase_label "Resetting PVE config artifacts")"
